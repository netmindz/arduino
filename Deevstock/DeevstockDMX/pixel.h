...
// Persistent local variable
  ...
  currLED = (currLED+1)	% (NUM_AUDIO_LEDS);                                           // Cycle through all the LED's. By Andrew Tuline.
  ...
  nblend(ledsAudio[currLED], newcolour, 192);                                        // Blend the old value and the new value for a gradual transitioning.
  showSegments();
} // pixel()
