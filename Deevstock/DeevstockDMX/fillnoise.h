...
// Persistent local variables
  ...
  if (sampleavg > NUM_AUDIO_LEDS) sampleavg = NUM_AUDIO_LEDS;                               // Clip the sampleavg to maximize at NUM_AUDIO_LEDS.
  
  for (int i= (NUM_AUDIO_LEDS-sampleavg/2)/2; i<(NUM_AUDIO_LEDS+sampleavg/2)/2; i++) {      // The louder the sound, the wider the soundbar.
    ...
    ledsAudio[i] = ColorFromPalette(currentPalette, index, sampleavg, LINEARBLEND);  // With that value, look up the 8 bit colour palette value and assign it to the current LED.
