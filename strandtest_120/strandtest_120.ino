...
void loop() {
  ...
  theaterChase2(strip.Color(0,   0, 127), strip.Color(0,   127, 0), 100); 
...
//Theatre-style crawling lights.
void theaterChase2(uint32_t c, uint32_t c2, uint8_t wait) {
  for (int j=0; j<10; j++) {  //do 10 cycles of chasing
    for (int q=0; q < 3; q++) {
      for (int i=0; i < strip.numPixels(); i=i+3) {
        strip.setPixelColor(i+q, c);    //turn every third pixel on
...
void theaterChaseRainbow(uint8_t wait, int loopcount) {
